...
void show(float p[], int grid[][SIDE]){//proietta e disegna
	for(int i=0; i<N; i++){
		float prj1[DIM-1];
		float p1[DIM];
		...
		for(int j=0; j<N; j++){
			...
			if(j!=i && abs(dist(p1, p2)-2) < TOL){
				...
				//printf("%f, %f\n", incx, incy);
				//if(incx==0||incy==0) printf("%d, %d  %d, %d, %d, %d, %d\n", x, y, x1, y1, dx, dy);
				float posx, posy;
				for(int w=1; w<d; w++){
					posx = incx * w;
					posy = incy * w;
...
float rYZ[] = {1, 0, 0, 0, cos(theta), -sin(theta), 0, sin(theta), cos(theta)};
