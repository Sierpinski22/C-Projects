...
void computeProjection2(float p[], float pr[]) {//proiezione dalla terza dimensione alla seconda
  float z = 1 / (3-p[3]);
  float projection[] = {z, 0, 0, 0, 0, z, 0, 0, 0, 0, z, 0};
  
  matMult(projection, 4, 3, p, 1, 4, pr);
}

void computeProjection(float p[], float pr[]) {//proiezione dalla terza dimensione alla seconda
  
  float projection2[3];
  computeProjection2(p, projection2);
  
  float z = 1 / (2-p[2]);
  float projection[] = {z, 0, 0, 0, z, 0};
  matMult(projection, 3, 2, projection2, 1, 3, pr);
}
...
float theta = 0.1;
float rho = 0.1;
float rXZ[] = {1, 0, 0, 0, 0, cos(theta), 0, -sin(theta), 0, 0, 1, 0, 0, sin(theta), 0, cos(theta)};
float rYW[] = {cos(rho), 0, -sin(rho), 0, 0, 1, 0, 0, sin(rho), 0, cos(rho), 0, 0, 0, 0, 1};
...
int main(){
	...
	while(1){//rotazioni successive. Sovrascrivere i punti e sopprimere il buffer?
		...
		matMult(points, DIM, N, rXZ, DIM, DIM, pointsBuffer);
		replacePoints(points, pointsBuffer);
		
		matMult(points, DIM, N, rYW, DIM, DIM, pointsBuffer);
		replacePoints(points, pointsBuffer);
	...
	for(int i=0; i<N; i++){
		printf("%f, %f, %f, %f\n", points[i*DIM], points[i*DIM+1] , points[i*DIM+2] , points[i*DIM+3]);
